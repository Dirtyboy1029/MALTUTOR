...
# @Time : 2024/1/7 15:43 
# @Author : DirtyBoy 
# @File : training_robust_maltutor.py
...
def Seq_experiment(val_type,
    ...
    if not os.path.isdir(robuts_model_path):
        ...
        val_data_filenames, val_gt_labels = utils.read_joblib(os.path.join(intermediate_data_saving_dir,
                                                                           data_type + '_test_database' + '.' + feature_type))
        ...
        easy_trainset, input_dim, dataX_np = feature_extractor.feature2ipt(easy_oos_features, easy_gt_labels,
                                                                           data_type=data_type)
        vanilla = Vanilla(architecture_type=architecture_type, model_directory=robuts_model_path)
